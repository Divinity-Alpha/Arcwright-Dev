...
FString SArcwrightDashboardPanel::GetServerPyPath() const
{
	...
	TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(TEXT("BlueprintLLM"));
	if (Plugin.IsValid())
	{
	...
	else
	{
		...
		PluginBaseDir = FPaths::Combine(FPaths::ProjectPluginsDir(), TEXT("BlueprintLLM"));
	}
...
TSharedRef<SWidget> SArcwrightDashboardPanel::BuildTierSection()
{
	TSharedRef<SWidget> Content =
		...
		[
			SNew(STextBlock)
			.Text(FText::FromString(TEXT("Licensed — All commands available")))
		...
		[
			SNew(STextBlock)
			.Text(FText::FromString(FString::Printf(TEXT("%d TCP commands  |  %d MCP tools"),
				FTierGating::TotalCommands, FTierGating::TotalMCPTools)))
			.ColorAndOpacity(ArcwrightColors::DimText)
			.Font(FCoreStyle::GetDefaultFontStyle("Regular", 12))
		]
		...
		[
			SNew(STextBlock)
			.Text(LOCTEXT("Version", "Arcwright v1.0.0 — The Bridge Between AI and Unreal Engine"))
			.ColorAndOpacity(ArcwrightColors::DimText)
			.Font(FCoreStyle::GetDefaultFontStyle("Regular", 11))
		];

	return BuildCard(LOCTEXT("AboutTitle", "ABOUT"), ArcwrightColors::AccentBlue, Content);
}
...
{
	FBlueprintLLMModule* Module = FModuleManager::GetModulePtr<FBlueprintLLMModule>("BlueprintLLM");
	...
	bool bConnectedSuccessfully)
{
	...
	{
		UE_LOG(LogBlueprintLLM, Warning, TEXT("Feedback HTTP POST failed (saved locally). Code: %d"),
...
FString SArcwrightDashboardPanel::GetFeedbackEndpoint() const
{
	...
	{
		GConfig->GetString(
			TEXT("/Script/BlueprintLLM.ArcwrightSettings"),
