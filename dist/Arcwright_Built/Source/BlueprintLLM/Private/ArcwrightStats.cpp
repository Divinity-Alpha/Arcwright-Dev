...
void FArcwrightStats::Initialize()
{
	...
	bInitialized = true;
	UE_LOG(LogBlueprintLLM, Log, TEXT("Arcwright Stats initialized (session %lld, %lld lifetime commands)"), TotalSessions, TotalCommands);
}
...
void FArcwrightStats::LoadFromDisk()
{
	...
	{
		UE_LOG(LogBlueprintLLM, Log, TEXT("No existing stats file at %s — starting fresh"), *FilePath);
		return;
	}
	...
	{
		UE_LOG(LogBlueprintLLM, Warning, TEXT("Failed to parse stats.json — starting fresh"));
		return;
	}
	...
	UE_LOG(LogBlueprintLLM, Log, TEXT("Loaded stats: %lld commands, %lld sessions, first use %s"), TotalCommands, TotalSessions, *FirstUseDate);
}
...
void FArcwrightStats::SaveToDisk()
{
	...
	{
		UE_LOG(LogBlueprintLLM, Warning, TEXT("Failed to save stats to %s"), *FilePath);
	}
}
