...
const FString FCommandServer::SERVER_VERSION = TEXT("1.0.4");
...
FCommandResult FCommandServer::HandleCompileBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Compiled and saved %s: %s (%d messages)"),
	...
	if (FPaths::FileExists(PIELogPath))
	{
		...
		for (int32 i = Start; i < AllLines.Num(); i++)
		{
			...
			if (AllLines[i].Contains(TEXT("BlueprintUserMessages")) ||
				AllLines[i].Contains(TEXT("LogArcwright")) ||
...
FCommandResult FCommandServer::HandleSetPirWidget(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("set_pir_widget: %s -> %s"), *WidgetName, *ConfigPath);
	return FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleCreateWidgetBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (UUserWidget* CDO = Cast<UUserWidget>(WBP->GeneratedClass->GetDefaultObject()))
	{
		...
		UE_LOG(LogArcwright, Log, TEXT("CreateWidgetBP DesignTimeSize: %dx%d (Custom mode)"), DesignWidth, DesignHeight);
	}
#endif
	...
	UE_LOG(LogArcwright, Log, TEXT("CreateWidgetBP SavePackage: %s -> %s"), *PackagePath, *PackageFilename);
	FSavePackageArgs SaveArgs;
	bool bSaved = SafeSavePackage(Package, WBP, PackageFilename, SaveArgs);
	UE_LOG(LogArcwright, Log, TEXT("CreateWidgetBP SavePackage result: %s"), bSaved ? TEXT("SUCCESS") : TEXT("FAILED"));
	...
	UE_LOG(LogArcwright, Log, TEXT("Created Widget Blueprint: %s at %s (saved=%d, layout=%d, design=%dx%d)"), *Name, *PackagePath, bSaved, LayoutScore, DesignWidth, DesignHeight);
	return FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleSetWidgetDesignSize(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("SetWidgetDesignSize: %s -> %dx%d (saved=%d)"), *Name, Width, Height, bSaved);
	return FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleProtectWidgetLayout(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("ProtectWidgetLayout: %s — %d protected, %d accessible (txt_*/Btn_*)"),
...
FCommandResult FCommandServer::HandleAddWidgetChild(const TSharedPtr<FJsonObject>& Params)
{
	...
	else
	{
		...
		if (!RootWidget)
		{
			...
			{
				RootCanvas->SetClipping(EWidgetClipping::ClipToBounds);
				UE_LOG(LogArcwright, Log, TEXT("AddWidgetChild: Set ClipToBounds on root CanvasPanel '%s'"), *WidgetName);
			}
		}
		else
		{
	...
	if (WBPPackage)
	{
		...
		// Ensure directory exists on disk
		IFileManager::Get().MakeDirectory(*FPaths::GetPath(PackageFilename), true);
		UE_LOG(LogArcwright, Log, TEXT("Widget SavePackage: Package=%s -> %s"), *PackagePath, *PackageFilename);
		FSavePackageArgs SaveArgs;
		bool bSaved = SafeSavePackage(WBPPackage, WBP, PackageFilename, SaveArgs);
		UE_LOG(LogArcwright, Log, TEXT("Widget SavePackage result: %s"), bSaved ? TEXT("SUCCESS") : TEXT("FAILED"));
	}
	...
	UE_LOG(LogArcwright, Log, TEXT("Added widget '%s' (%s) to %s (layout score: %d)"), *WidgetName, *WidgetType, *WBPName, LayoutScore);
	return FCommandResult::Ok(Data);
}
...
static FLinearColor ParseLinearColor(const FString& Value)
{
	...
	if (Trimmed.StartsWith(TEXT("hex:"), ESearchCase::IgnoreCase))
	{
		...
		UE_LOG(LogArcwright, Warning, TEXT("ParseLinearColor: invalid hex format '%s', need >=6 hex chars"), *Hex);
	}
...
FCommandResult FCommandServer::HandleSetWidgetProperty(const TSharedPtr<FJsonObject>& Params)
{
		...
		if (!bIsArcwrightWidget)
		{
			...
			if (!bForce)
			{
				FString ErrMsg = FString::Printf(
					...
					*WBPName, *PackagePath);
				UE_LOG(LogArcwright, Warning, TEXT("set_widget_property BLOCKED: %s"), *ErrMsg);
				return FCommandResult::Error(ErrMsg);
			}
			else
			{
				UE_LOG(LogArcwright, Warning,
	...
	UE_LOG(LogArcwright, Log, TEXT("Set property '%s' on widget '%s' in %s (layout: %d)"), *PropertyName, *WidgetName, *WBPName, LayoutScore);
	return FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleReparentWidgetBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	...
	for (const FName& FuncName : FunctionsToRemove)
	{
		for (int32 i = WBP->FunctionGraphs.Num() - 1; i >= 0; --i)
		{
			...
			{
				UE_LOG(LogArcwright, Log, TEXT("ReparentWidget: Removing conflicting function '%s' from %s"), *FuncName.ToString(), *WBPName);
	...
	UE_LOG(LogArcwright, Log, TEXT("Reparented widget '%s': %s -> %s (%d conflicts resolved, compiled=%d)"),
		...
		if (Pair.Value->TryGetString(ValueStr))
		{
			...
			else if (FClassProperty* ClassProp = CastField<FClassProperty>(Prop))
			{
				...
				if (ClassValue)
				{
					...
					SetProperties.Add(Key);
					UE_LOG(LogArcwright, Log, TEXT("set_class_defaults: %s = %s (class)"), *Key, *ClassValue->GetName());
				}
				else
				{
					UE_LOG(LogArcwright, Warning, TEXT("set_class_defaults: class not found for '%s': %s"), *Key, *ValueStr);
				}
			}
			...
			else if (FObjectProperty* ObjProp = CastField<FObjectProperty>(Prop))
			{
				...
				if (ObjValue)
				{
					...
					SetProperties.Add(Key);
					UE_LOG(LogArcwright, Log, TEXT("set_class_defaults: %s = %s (object)"), *Key, *ObjValue->GetName());
				}
				else
				{
					UE_LOG(LogArcwright, Warning, TEXT("set_class_defaults: object not found for '%s': %s"), *Key, *ValueStr);
				}
			}
			else
			{
...
FCommandResult FCommandServer::HandleAddScrollSync(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("add_scroll_sync: %s -> %d targets"), *SourceSB, TargetSBs.Num());
	return FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleBindTextToVariable(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("bind_text_to_variable: %s.Text -> %s via %s"), *TextWidget, *VarName, *BindFuncName);
	return FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleSetWidgetIsVariable(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("set_widget_is_variable: %s.%s = %s"), *WBPName, *WidgetName, bIsVariable ? TEXT("true") : TEXT("false"));
	return FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleAddWidgetVariable(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("add_widget_variable: %s.%s (%s)"), *WBPName, *VarName, *VarType);
	return FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleSetWidgetEntryClass(const TSharedPtr<FJsonObject>& Params)
{
	...
	Data->SetStringField(TEXT("widget_name"), WidgetName);
	UE_LOG(LogArcwright, Log, TEXT("set_widget_entry_class: %s -> %s"), *WidgetName, *EntryClass);
	return FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleAssignMediaTexture(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Assigned media texture: %s -> %s"), *MediaTexPath, *WidgetName);
	return FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleImportFontFace(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Imported font face: %s -> %s"), *TtfPath, *FinalPath);
	return FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleCreateFontAsset(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Created font asset: %s"), *PackagePath);
	return FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleAddFontTypeface(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Added typeface '%s' to %s"), *TypefaceName, *FontAsset);
	return FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleImportFontFamily(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Imported font family: %s (%d typefaces)"), *FamilyName, ImportedArray.Num());
	return FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleReparentBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Reparented %s: %s -> %s (compiled: %s)"),
