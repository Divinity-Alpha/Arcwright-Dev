...
public:
	...
	/** Create a single node from a DSL definition. Used by add_node command. */
	static UK2Node* CreateNodeFromDef(UBlueprint* BP, UEdGraph* Graph, const FDSLNode& NodeDef);

	/** Smart pin resolver — maps DSL pin names to UE internal pin names. Used by add_connection/set_node_param. */
	static UEdGraphPin* FindPinByDSLName(UEdGraphNode* Node, const FString& DSLName, EEdGraphPinDirection Direction);
