...
// Static delegate definition for TierGating
FOnLicenseStateChanged FTierGating::OnLicenseStateChanged;
...
bool FCommandServer::Start(int32 Port)
{
	if (bRunning)
	{
		UE_LOG(LogArcwright, Warning, TEXT("Command server already running"));
		return true;
	}
	...
	if (!Listener->Init())
	{
		UE_LOG(LogArcwright, Error, TEXT("Failed to bind TCP listener on port %d"), Port);
		delete Listener;
		Listener = nullptr;
		return false;
	}
		...
		SlatePostTickHandle = FSlateApplication::Get().OnPostTick().AddLambda([this](float DeltaTime)
		{
			if (bPIERequested.exchange(false))
			{
				if (GEditor && !GEditor->PlayWorld)
				{
					...
					UE_LOG(LogArcwright, Log, TEXT("PIE: RequestPlaySession called from Slate OnPostTick"));
				}
			}
		});
		UE_LOG(LogArcwright, Log, TEXT("Arcwright: Registered Slate post-tick callback for PIE"));
	}
...
void FCommandServer::Stop()
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Arcwright Command Server stopped"));
}
...
bool FCommandServer::OnConnectionAccepted(FSocket* ClientSocket, const FIPv4Endpoint& ClientEndpoint)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Client connected: %s"), *ClientEndpoint.ToString());

	// Configure socket for persistent, high-throughput connections
	ClientSocket->SetNoDelay(true);           // Disable Nagle — send responses immediately
	ClientSocket->SetNonBlocking(false);      // Blocking mode for this thread
	int32 SendBufSize = 262144;               // 256 KB send buffer
	int32 RecvBufSize = 262144;               // 256 KB recv buffer
	...
	Async(EAsyncExecution::Thread, [this, ClientSocket]()
	{
		...
		UE_LOG(LogArcwright, Log, TEXT("Client disconnected"));
	});
...
void FCommandServer::ProcessClient(FSocket* ClientSocket)
{
	while (bRunning)
	{
		...
		UE_LOG(LogArcwright, Log, TEXT("Received: %s"), *Line);
		...
		UE_LOG(LogArcwright, Log, TEXT("Response: %s — %s"),
...
{
	// Chunked read buffer — reads up to 4096 bytes at a time instead of 1 byte per syscall.
	// Scans for newline in the received chunk, returns the line, and leaves leftover bytes
	// for the next call via the static thread-local approach (each client runs on its own thread).
	// NOTE: This is safe because each FSocket* is only processed by one thread.

	static thread_local TMap<FSocket*, TArray<uint8>> SocketBuffers;
	TArray<uint8>& Overflow = SocketBuffers.FindOrAdd(Socket);

	TArray<uint8> LineBuffer;

	// First, check if we already have a complete line in the overflow from a previous read
	for (int32 i = 0; i < Overflow.Num(); i++)
	{
		if (Overflow[i] == '\n')
		{
			LineBuffer.Append(Overflow.GetData(), i);
			Overflow.RemoveAt(0, i + 1, EAllowShrinking::No);
			if (LineBuffer.Num() == 0) return FString();
			LineBuffer.Add(0);
			return FString(UTF8_TO_TCHAR(LineBuffer.GetData()));
		}
	}

	// Move any overflow into our line buffer
	LineBuffer.Append(Overflow);
	Overflow.Empty();
	...
	while (bRunning)
	{
		// Wait for data — 5 second timeout per iteration
		if (!Socket->Wait(ESocketWaitConditions::WaitForRead, FTimespan::FromSeconds(5.0)))
		{
			ConsecutiveTimeouts++;
			// Only give up after 60 seconds of total silence (12 × 5s)
			// This prevents false disconnects from brief scheduling gaps
			if (ConsecutiveTimeouts >= 12)
			{
				UE_LOG(LogArcwright, Warning, TEXT("Client silent for 60s, dropping connection"));
		...
		if (!Socket->Recv(ChunkBuf, sizeof(ChunkBuf), BytesRead))
		{
			SocketBuffers.Remove(Socket);
			return FString();
		}
		...
		for (int32 i = 0; i < BytesRead; i++)
		{
			if (ChunkBuf[i] == '\n')
			{
				// Everything before the newline is part of this line
				LineBuffer.Append(ChunkBuf, i);
				// Everything after goes into overflow for next call
				int32 Remaining = BytesRead - i - 1;
				if (Remaining > 0)
				{
					Overflow.Append(ChunkBuf + i + 1, Remaining);
				}
		...
		LineBuffer.Append(ChunkBuf, BytesRead);

		// Safety: reject lines longer than 16 MB (malformed client)
		if (LineBuffer.Num() > 16 * 1024 * 1024)
		{
			UE_LOG(LogArcwright, Error, TEXT("Client sent line exceeding 16 MB, dropping connection"));
			SocketBuffers.Remove(Socket);
			return FString();
		}
	}
...
void FCommandServer::SendResponse(FSocket* Socket, const FCommandResult& Result)
{
	...
	int32 BytesSentSoFar = 0;

	while (BytesSentSoFar < TotalBytes)
	{
		int32 BytesSent = 0;
		if (!Socket->Send(Data + BytesSentSoFar, TotalBytes - BytesSentSoFar, BytesSent))
		{
			UE_LOG(LogArcwright, Warning, TEXT("Socket send failed after %d/%d bytes"), BytesSentSoFar, TotalBytes);
			break;
		}
		if (BytesSent <= 0)
		{
			// Wait briefly for send buffer to drain, then retry
			if (!Socket->Wait(ESocketWaitConditions::WaitForWrite, FTimespan::FromSeconds(5.0)))
			{
				UE_LOG(LogArcwright, Warning, TEXT("Socket send buffer full, dropping after %d/%d bytes"), BytesSentSoFar, TotalBytes);
				break;
			}
			continue;
		}
...
{
	// ── Tier gating ─────────────────────────────────────────
	if (!FTierGating::IsCommandAllowed(Command))
	{
		TSharedPtr<FJsonObject> ErrorData = MakeShareable(new FJsonObject());
		ErrorData->SetStringField(TEXT("error"), TEXT("pro_required"));
		ErrorData->SetStringField(TEXT("command"), Command);
		ErrorData->SetStringField(TEXT("upgrade_url"), TEXT("https://arcwright.app"));

		FString Msg = FString::Printf(
			TEXT("The command '%s' requires Arcwright Pro. Upgrade at arcwright.app"),
			*Command);
	...
	if (Command == TEXT("create_sound_from_dsl"))       { return HandleCreateSoundFromDSL(Params); }
...
FCommandResult FCommandServer::BuildBlueprintFromIR(FDSLBlueprint& DSL)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Imported %s: %d/%d nodes, %d/%d connections, compiled=%s"),
...
FCommandResult FCommandServer::HandleCompileBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Compiled %s: %s (%d messages)"),
...
bool FCommandServer::DeleteExistingBlueprint(const FString& Name)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Deleting existing Blueprint: %s"), *Name);
	...
	if (NumDeleted > 0)
	{
		UE_LOG(LogArcwright, Log, TEXT("Deleted Blueprint: %s"), *Name);
		return true;
	}

	UE_LOG(LogArcwright, Warning, TEXT("Failed to delete Blueprint: %s"), *Name);
	return false;
}
...
UEdGraphNode* FCommandServer::FindNodeInGraph(UEdGraph* Graph, const FString& NodeId)
{
	...
	// Try matching by node_N index format (from get_blueprint_info)
	...
	// Try matching by node title (case-insensitive substring)
	for (UEdGraphNode* Node : Graph->Nodes)
	{
...
FCommandResult FCommandServer::HandleAddNode(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		TempNode.UEClass = TEXT("UK2Node_MacroInstance");
	...
	else if (NodeType.StartsWith(TEXT("CastTo")))
	{
		...
		TempNode.CastClass = NodeType.Mid(6); // Remove "CastTo" prefix
	}
	// Variables
	else if (NodeType == TEXT("GetVar"))
	{
		TempNode.UEClass = TEXT("UK2Node_VariableGet");
	}
	else if (NodeType == TEXT("SetVar"))
	{
	...
	UE_LOG(LogArcwright, Log, TEXT("Added node %s (%s) to %s"),
...
FCommandResult FCommandServer::HandleRemoveNode(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Removed node %s from %s"), *NodeID, *BlueprintName);
...
FCommandResult FCommandServer::HandleAddConnection(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Connected %s.%s -> %s.%s in %s"),
...
FCommandResult FCommandServer::HandleRemoveConnection(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Disconnected %s.%s -> %s.%s in %s"),
		*SrcNodeID, *SrcPinName, *DstNodeID, *DstPinName, *BlueprintName);
...
FCommandResult FCommandServer::HandleAddNodesBatch(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("add_nodes_batch on %s: %d/%d succeeded"),
...
FCommandResult FCommandServer::HandleAddConnectionsBatch(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("add_connections_batch on %s: %d/%d succeeded"),
...
FCommandResult FCommandServer::HandleValidateBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("validate_blueprint %s: %d errors, %d warnings, %d info"),
...
FCommandResult FCommandServer::HandleSetNodeParam(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Set %s.%s = %s in %s"),
...
FCommandResult FCommandServer::HandleSetVariableDefault(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Set variable %s default = %s in %s"),
...
FCommandResult FCommandServer::HandleSpawnActorAt(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!MeshPath.IsEmpty())
	{
		...
		if (SMActor && SMActor->GetStaticMeshComponent())
		{
			...
			if (Mesh)
			{
				...
				UE_LOG(LogArcwright, Log, TEXT("Set mesh to %s"), *MeshPath);
			}
			else
			{
				UE_LOG(LogArcwright, Warning, TEXT("Could not load mesh: %s"), *MeshPath);
			}
		}
	}
	...
	if (!MaterialPath.IsEmpty())
	{
		...
		if (Mat)
		{
			...
			if (SMActor && SMActor->GetStaticMeshComponent())
			{
				...
				UE_LOG(LogArcwright, Log, TEXT("Set material to %s"), *MaterialPath);
			}
		}
	}
	...
	UE_LOG(LogArcwright, Log, TEXT("Spawned %s at (%s) label=%s"),
...
FCommandResult FCommandServer::HandleSetActorTransform(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Updated transform for %s"), *Label);
...
FCommandResult FCommandServer::HandleDeleteActor(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Delete actor %s: %s"), *Label,
...
FCommandResult FCommandServer::HandleAddComponent(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Added component '%s' (%s) to %s"),
...
FCommandResult FCommandServer::HandleRemoveComponent(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Removed component '%s' from %s"), *ComponentName, *BlueprintName);
	...
	if (Prop)
	{
		...
		if (Value->TryGetBool(bJsonBool))
		{
			if (FBoolProperty* BoolProp = CastField<FBoolProperty>(Prop))
			{
				...
				UE_LOG(LogArcwright, Log, TEXT("Set bool property '%s' = %s via reflection"), *PropertyName, bJsonBool ? TEXT("true") : TEXT("false"));
				return true;
			}
		}
		...
		if (Value->TryGetNumber(NumVal))
		{
			if (FFloatProperty* FloatProp = CastField<FFloatProperty>(Prop))
			{
				...
				UE_LOG(LogArcwright, Log, TEXT("Set float property '%s' = %f via reflection"), *PropertyName, (float)NumVal);
				return true;
			}
...
FCommandResult FCommandServer::HandleSetComponentProperty(const TSharedPtr<FJsonObject>& Params)
{
		...
		if (CDO)
		{
			...
			for (UActorComponent* Comp : CDOComps)
			{
				if (Comp->GetName().Contains(ComponentName))
				{
					...
					UE_LOG(LogArcwright, Log, TEXT("Also set material on CDO component %s"), *Comp->GetName());
					break;
				}
			}
		}
	...
	UE_LOG(LogArcwright, Log, TEXT("Set %s.%s in %s"), *ComponentName, *PropertyName, *BlueprintName);
...
FCommandResult FCommandServer::HandleCreateMaterialInstance(const TSharedPtr<FJsonObject>& Params)
{
	...
	FString PackagePath = FString::Printf(TEXT("/Game/Arcwright/Materials/%s"), *Name);
	{
		...
		if (ExistingObj)
		{
			...
			UE_LOG(LogArcwright, Log, TEXT("Deleted existing material instance before recreation: %s"), *Name);
		}
	}
	...
	UE_LOG(LogArcwright, Log, TEXT("Created MaterialInstance: %s (parent: %s, %d scalar, %d vector params)"),
...
FCommandResult FCommandServer::HandleCreateSimpleMaterial(const TSharedPtr<FJsonObject>& Params)
{
	...
	FString PackagePath = FString::Printf(TEXT("/Game/Arcwright/Materials/%s"), *Name);
	{
		...
		if (ExistingMat)
		{
			...
			ObjectTools::ForceDeleteObjects(ToDelete, false);
			CollectGarbage(GARBAGE_COLLECTION_KEEPFLAGS);
			UE_LOG(LogArcwright, Log, TEXT("Deleted existing material before recreation: %s"), *Name);
		}
	}
	...
	UE_LOG(LogArcwright, Log, TEXT("Created simple material: %s (color: %.2f, %.2f, %.2f, emissive: %.1f)"),
...
FCommandResult FCommandServer::HandleCreateTexturedMaterial(const TSharedPtr<FJsonObject>& Params)
{
	...
	// Delete existing material if present (Lesson #37 — prevents "partially loaded" crash)
	{
		...
		if (ExistingMat)
		{
			...
			ObjectTools::ForceDeleteObjects(ToDelete, false);
			CollectGarbage(GARBAGE_COLLECTION_KEEPFLAGS);
			UE_LOG(LogArcwright, Log, TEXT("Deleted existing textured material before recreation: %s"), *Name);
		}
	}
	...
	UE_LOG(LogArcwright, Log, TEXT("Created textured material: %s (texture: %s, roughness: %.2f, metallic: %.2f, tiling: %.1f)"),
...
UMaterialInterface* FCommandServer::ResolveMaterialByName(const FString& NameOrPath, FString& OutResolvedPath, FString& OutError)
{
	...
	for (const FString& Path : SearchPaths)
	{
		...
		if (Mat)
		{
			...
			UE_LOG(LogArcwright, Log, TEXT("Resolved material '%s' → '%s'"), *NameOrPath, *Path);
			return Mat;
		}
	}
	...
	if (ExactMatches.Num() == 1)
	{
		...
		if (Mat)
		{
			...
			UE_LOG(LogArcwright, Log, TEXT("Resolved material '%s' → '%s' (exact registry match)"), *NameOrPath, *FullPath);
			return Mat;
		}
	}
	if (ExactMatches.Num() > 1)
	...
	if (ContainsMatches.Num() == 1)
	{
		...
		{
			OutResolvedPath = FullPath;
			UE_LOG(LogArcwright, Log, TEXT("Resolved material '%s' → '%s' (fuzzy registry match)"), *NameOrPath, *FullPath);
			return Mat;
		}
	}
	if (ContainsMatches.Num() > 1)
...
FCommandResult FCommandServer::HandleApplyMaterial(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (PostCompileNode)
	{
		...
		{
			UE_LOG(LogArcwright, Warning, TEXT("SCS template lost material after compile — re-applying"));
	...
	UE_LOG(LogArcwright, Log, TEXT("Applied material %s to %s.%s[%d] in %s"),
...
FCommandResult FCommandServer::HandleSetActorMaterial(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Set material %s on actor %s component %s[%d]"),
...
FCommandResult FCommandServer::HandleSaveAll(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("SaveAll (maps=%s): %s"),
	...
	{
		UE_LOG(LogArcwright, Log, TEXT("SaveAll: saved %d external actor packages (World Partition)"), ExternalActorsSaved);
	}
...
FCommandResult FCommandServer::HandleSaveLevel(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (bIsUntitled)
	{
		...
		UE_LOG(LogArcwright, Log, TEXT("SaveLevel (new) %s -> %s: %s"), *MapName, *FilePath, bResult ? TEXT("success") : TEXT("failed"));
	}
	else
	{
		...
		UE_LOG(LogArcwright, Log, TEXT("SaveLevel %s: %s"), *MapName, bResult ? TEXT("success") : TEXT("failed"));
	}
	...
	UE_LOG(LogArcwright, Log, TEXT("SaveLevel: flushed all dirty packages (WP external actors)"));
...
FCommandResult FCommandServer::HandleDuplicateBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Duplicated %s -> %s at %s"), *SourceName, *NewName, *NewAssetPath);
...
FCommandResult FCommandServer::HandleStopPlay(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (bWasPlaying)
	{
		...
		UE_LOG(LogArcwright, Log, TEXT("PIE stop requested"));
	}
...
FCommandResult FCommandServer::HandleAddAudioComponent(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (AudioComp)
	{
		...
		if (!SoundPath.IsEmpty())
		{
			...
			{
				UE_LOG(LogArcwright, Warning, TEXT("Sound not found: %s — component created without sound"), *SoundPath);
			}
		}
	}
...
FCommandResult FCommandServer::HandleAddNiagaraComponent(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (NiagaraComp)
	{
		...
		if (!SystemPath.IsEmpty())
		{
			...
			{
				UE_LOG(LogArcwright, Warning, TEXT("Niagara system not found: %s — component created without system"), *SystemPath);
			}
		}
	}
...
FCommandResult FCommandServer::HandleQuitEditor(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		GEditor->RequestEndPlayMap();
		UE_LOG(LogArcwright, Log, TEXT("quit_editor: Stopped PIE session"));
	}
	...
	if (bSaveFirst)
	{
		...
		UE_LOG(LogArcwright, Log, TEXT("quit_editor: Saved all dirty packages + %d external actor packages"), ExternalActorsSaved);
	}
	...
	UE_LOG(LogArcwright, Log, TEXT("quit_editor: Requesting clean exit"));
...
FCommandResult FCommandServer::HandleCreateWidgetBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Created Widget Blueprint: %s at %s"), *Name, *PackagePath);
	return FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleAddWidgetChild(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Added widget '%s' (%s) to %s"), *WidgetName, *WidgetType, *WBPName);
	return FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleSetWidgetProperty(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Set property '%s' on widget '%s' in %s"), *PropertyName, *WidgetName, *WBPName);
	return FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleRemoveWidget(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Removed widget '%s' from %s"), *WidgetName, *WBPName);
	return FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleSetBlackboardKeyDefault(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (TypeName.Contains(TEXT("Vector")))
	{
		...
		UE_LOG(LogArcwright, Log, TEXT("set_blackboard_key_default: Vector key '%s' — needs runtime assignment"), *KeyName);
		return FCommandResult::Ok(Data);
	}
...
FCommandResult FCommandServer::HandleSetClassDefaults(const TSharedPtr<FJsonObject>& Params)
{
	...
	if ((*PropertiesObj)->TryGetStringField(TEXT("ai_controller_class"), AIControllerClassName))
	{
		...
		if (PawnCDO)
		{
			...
			if (ControllerClass && ControllerClass->IsChildOf(AAIController::StaticClass()))
			{
				...
				UE_LOG(LogArcwright, Log, TEXT("Set AIControllerClass to %s on %s"), *ControllerClass->GetName(), *BlueprintName);
			}
			else
			{
		...
		if (AICDO)
		{
			...
			if (BTProp)
			{
				...
				if (ObjProp)
				{
					...
					UE_LOG(LogArcwright, Log, TEXT("Set DefaultBehaviorTree to %s on %s"), *BehaviorTreeName, *BlueprintName);
				}
			}
			else
			{
				UE_LOG(LogArcwright, Warning, TEXT("DefaultBehaviorTree property not found on AIController CDO. Will try RunBehaviorTree approach."));
	...
	if ((*PropertiesObj)->TryGetStringField(TEXT("default_pawn_class"), DefaultPawnClassName))
	{
		...
		if (GMCDO)
		{
			...
			if (PawnClass && PawnClass->IsChildOf(APawn::StaticClass()))
			{
				...
				UE_LOG(LogArcwright, Log, TEXT("Set DefaultPawnClass to %s on %s"), *PawnClass->GetName(), *BlueprintName);
			}
			else
			{
	...
	if ((*PropertiesObj)->TryGetStringField(TEXT("player_controller_class"), PlayerControllerClassName))
	{
		...
		if (GMCDO)
		{
			...
			if (ControllerClass && ControllerClass->IsChildOf(APlayerController::StaticClass()))
			{
				...
				UE_LOG(LogArcwright, Log, TEXT("Set PlayerControllerClass to %s on %s"), *ControllerClass->GetName(), *BlueprintName);
			}
			else
			{
	...
	for (const auto& Pair : (*PropertiesObj)->Values)
	{
		...
		if (!Prop)
		{
			UE_LOG(LogArcwright, Warning, TEXT("set_class_defaults: property '%s' not found on CDO of %s"), *Key, *BlueprintName);
			continue;
		}
		...
		if (Pair.Value->TryGetString(ValueStr))
		{
			...
			if (FBoolProperty* BoolProp = CastField<FBoolProperty>(Prop))
			{
				...
				UE_LOG(LogArcwright, Log, TEXT("set_class_defaults: %s = %s (bool)"), *Key, bVal ? TEXT("true") : TEXT("false"));
			}
			...
			else if (FByteProperty* ByteProp = CastField<FByteProperty>(Prop))
			{
				if (ByteProp->Enum)
				{
					...
					if (EnumVal != INDEX_NONE)
					{
						...
						SetProperties.Add(Key);
						UE_LOG(LogArcwright, Log, TEXT("set_class_defaults: %s = %s (enum %lld)"), *Key, *ValueStr, EnumVal);
					}
					else
					{
						UE_LOG(LogArcwright, Warning, TEXT("set_class_defaults: enum value '%s' not found for %s"), *ValueStr, *Key);
					}
				}
				else
				{
			...
			{
				UE_LOG(LogArcwright, Warning, TEXT("set_class_defaults: unsupported property type for '%s' — %s"), *Key, *Prop->GetClass()->GetName());
			}
		}
	}
...
FCommandResult FCommandServer::HandleSetupAIForPawn(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (!ControllerBP)
	{
		...
		UE_LOG(LogArcwright, Log, TEXT("Created AIController Blueprint: %s"), *ControllerName);
	}
	else
	{
		UE_LOG(LogArcwright, Log, TEXT("Reusing existing AIController Blueprint: %s"), *ControllerName);
	}
	...
	if (bCreatedNew)
	{
		...
		if (EventGraph)
		{
			...
			{
				UE_LOG(LogArcwright, Warning, TEXT("RunBehaviorTree function not found on AAIController"));
			}
		}
	...
	UE_LOG(LogArcwright, Log, TEXT("Setup AI complete: %s -> %s -> %s"),
...
FCommandResult FCommandServer::HandleImportStaticMesh(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (ExistingMesh)
	{
		...
		UE_LOG(LogArcwright, Log, TEXT("Static mesh already exists: %s — returning existing. Delete first to re-import."), *ExistingMesh->GetPathName());
		return FCommandResult::Ok(Data);
	}
	...
	if (Mesh)
	{
		...
		UE_LOG(LogArcwright, Log, TEXT("Imported static mesh: %s from %s"), *Mesh->GetPathName(), *FilePath);
	}
	else
	{
...
FCommandResult FCommandServer::HandleImportTexture(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (Texture)
	{
		...
		UE_LOG(LogArcwright, Log, TEXT("Imported texture: %s (%dx%d) from %s"),
...
FCommandResult FCommandServer::HandleImportSound(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (Sound)
	{
		...
		UE_LOG(LogArcwright, Log, TEXT("Imported sound: %s (%.2fs, %d ch) from %s"),
...
FCommandResult FCommandServer::HandleCreateSplineActor(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Created spline actor %s with %d points"), *Name, PointsArray->Num());
...
FCommandResult FCommandServer::HandleAddPostProcessVolume(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Spawned PostProcessVolume '%s' at %s (infinite=%d)"),
...
FCommandResult FCommandServer::HandleSetPostProcessSettings(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Set %d post-process settings on '%s'"), SettingsApplied, *Label);
...
FCommandResult FCommandServer::HandleSetMovementDefaults(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Set %d movement properties on %s (%s)"), PropsSet, *BPName, *CompType);
...
FCommandResult FCommandServer::HandleAddPhysicsConstraint(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Created %s constraint '%s' between %s and %s"),
...
FCommandResult FCommandServer::HandleBreakConstraint(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Broke constraint '%s'"), *Label);
...
FCommandResult FCommandServer::HandleCreateSequence(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Created sequence '%s' (%.1fs)"), *Name, Duration);
...
FCommandResult FCommandServer::HandleAddSequenceTrack(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Added %s track for '%s' in sequence '%s'"), *TrackType, *ActorLabel, *SeqName);
...
FCommandResult FCommandServer::HandleAddKeyframe(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Added %d keyframe(s) at %.2fs for '%s' %s track in '%s'"),
...
FCommandResult FCommandServer::HandleSetLandscapeMaterial(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Set landscape material to '%s'"), *MaterialPath);
...
FCommandResult FCommandServer::HandleCreateFoliageType(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		UE_LOG(LogArcwright, Warning, TEXT("Mesh not found: %s — foliage type created without mesh"), *MeshPath);
	}
	...
	UE_LOG(LogArcwright, Log, TEXT("Created foliage type '%s' mesh='%s' density=%.0f scale=[%.1f,%.1f]"),
...
FCommandResult FCommandServer::HandlePaintFoliage(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Painted %d foliage instances of '%s' at (%.0f,%.0f,%.0f) r=%.0f"),
...
FCommandResult FCommandServer::HandleSetupSceneLighting(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (DirLight)
	{
		...
		UE_LOG(LogArcwright, Log, TEXT("Created DirectionalLight: intensity=%.1f, pitch=%.1f"), DirIntensity, DirPitch);
	}
	...
	if (Sky)
	{
		...
		UE_LOG(LogArcwright, Log, TEXT("Created SkyLight: intensity=%.1f"), SkyIntensity);
	}
		...
		if (!bHasAtmo)
		{
			...
			if (AtmoClass)
			{
				...
				if (Atmo)
				{
					...
					UE_LOG(LogArcwright, Log, TEXT("Created SkyAtmosphere"));
				}
			}
		}
	}
	...
	if (bAddFog)
	{
		...
		if (Fog)
		{
			...
			UE_LOG(LogArcwright, Log, TEXT("Created ExponentialHeightFog: density=%.4f"), FogDensity);
		}
	}
...
FCommandResult FCommandServer::HandleSetGameMode(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Set world GameMode to %s"), *GMClass->GetName());
...
FCommandResult FCommandServer::HandleRenameAsset(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Renamed asset: %s -> %s"), *OldName, *NewName);
...
FCommandResult FCommandServer::HandleReparentBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("Reparented %s: %s -> %s"), *BPName, *OldParent, *NewParentClass->GetName());
...
FCommandResult FCommandServer::HandleSpawnActorGrid(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("SpawnActorGrid: %d actors (%dx%d) of class %s"),
...
FCommandResult FCommandServer::HandleSpawnActorCircle(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("SpawnActorCircle: %d actors (radius=%.0f) of class %s"),
...
FCommandResult FCommandServer::HandleSpawnActorLine(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("SpawnActorLine: %d actors from (%s) to (%s) of class %s"),
...
FCommandResult FCommandServer::HandleBatchScaleActors(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("BatchScaleActors: scaled %d actors (mode=%s)"), Scaled, *Mode);
...
FCommandResult FCommandServer::HandleBatchMoveActors(const TSharedPtr<FJsonObject>& Params)
{
	...
	UE_LOG(LogArcwright, Log, TEXT("BatchMoveActors: moved %d actors (mode=%s)"), Moved, *Mode);
...
// ── create_sound_from_dsl ─────────────────────────────────────
// Receives Sound DSL elements array and dispatches to individual sound commands.
// Follows same pattern as create_gas_from_dsl / create_quest_from_dsl.
FCommandResult FCommandServer::HandleCreateSoundFromDSL(const TSharedPtr<FJsonObject>& Params)
{
	// Accept either "dsl" (raw text → caller should pre-parse) or "elements" (pre-parsed array)
	const TArray<TSharedPtr<FJsonValue>>* Elements = nullptr;
	if (Params->HasField(TEXT("elements")))
	{
		Elements = &Params->GetArrayField(TEXT("elements"));
	}
	if (!Elements || Elements->Num() == 0)
	{
		return FCommandResult::Error(TEXT("Missing or empty 'elements' array. Provide pre-parsed sound elements."));
	}

	FString DesignName = Params->HasField(TEXT("name")) ? Params->GetStringField(TEXT("name")) : TEXT("SoundDesign");
	...
	{
		TSharedPtr<FJsonObject> Elem = ElemVal->AsObject();
		if (!Elem.IsValid()) { Failed++; continue; }

		FString ElemType = Elem->GetStringField(TEXT("type")).ToUpper();
		FString ElemName = Elem->GetStringField(TEXT("name"));

		FCommandResult SubResult;

		if (ElemType == TEXT("SOUND_CLASS"))
		{
			TSharedPtr<FJsonObject> P = MakeShareable(new FJsonObject());
			P->SetStringField(TEXT("name"), ElemName);
			if (Elem->HasField(TEXT("volume"))) P->SetNumberField(TEXT("volume"), Elem->GetNumberField(TEXT("volume")));
			if (Elem->HasField(TEXT("pitch"))) P->SetNumberField(TEXT("pitch"), Elem->GetNumberField(TEXT("pitch")));
			if (Elem->HasField(TEXT("parent_class"))) P->SetStringField(TEXT("parent_class"), Elem->GetStringField(TEXT("parent_class")));
			SubResult = HandleCreateSoundClass(P);
		}
		else if (ElemType == TEXT("ATTENUATION"))
		{
			TSharedPtr<FJsonObject> P = MakeShareable(new FJsonObject());
			P->SetStringField(TEXT("name"), ElemName);
			if (Elem->HasField(TEXT("inner_radius"))) P->SetNumberField(TEXT("inner_radius"), Elem->GetNumberField(TEXT("inner_radius")));
			if (Elem->HasField(TEXT("outer_radius"))) P->SetNumberField(TEXT("outer_radius"), Elem->GetNumberField(TEXT("outer_radius")));
			if (Elem->HasField(TEXT("falloff_distance"))) P->SetNumberField(TEXT("outer_radius"), Elem->GetNumberField(TEXT("falloff_distance")));
			if (Elem->HasField(TEXT("spatialization"))) P->SetBoolField(TEXT("spatialization"), Elem->GetBoolField(TEXT("spatialization")));
			SubResult = HandleCreateAttenuationSettings(P);
		}
		else if (ElemType == TEXT("REVERB_ZONE"))
		{
			TSharedPtr<FJsonObject> P = MakeShareable(new FJsonObject());
			if (Elem->HasField(TEXT("location"))) P->SetObjectField(TEXT("location"), Elem->GetObjectField(TEXT("location")));
			FString Preset = Elem->HasField(TEXT("reverb_effect")) ? Elem->GetStringField(TEXT("reverb_effect")) :
							 Elem->HasField(TEXT("reverb_preset")) ? Elem->GetStringField(TEXT("reverb_preset")) : TEXT("None");
			P->SetStringField(TEXT("reverb_preset"), Preset);
			P->SetStringField(TEXT("label"), ElemName);
			SubResult = HandleCreateAudioVolume(P);
		}
		else if (ElemType == TEXT("AMBIENT"))
		{
			TSharedPtr<FJsonObject> P = MakeShareable(new FJsonObject());
			FString Sound = Elem->HasField(TEXT("sound_asset")) ? Elem->GetStringField(TEXT("sound_asset")) :
							Elem->HasField(TEXT("sound")) ? Elem->GetStringField(TEXT("sound")) : TEXT("");
			P->SetStringField(TEXT("sound_asset"), Sound);
			if (Elem->HasField(TEXT("location"))) P->SetObjectField(TEXT("location"), Elem->GetObjectField(TEXT("location")));
			if (Elem->HasField(TEXT("auto_play"))) P->SetBoolField(TEXT("auto_play"), Elem->GetBoolField(TEXT("auto_play")));
			if (Elem->HasField(TEXT("attenuation"))) P->SetStringField(TEXT("attenuation"), Elem->GetStringField(TEXT("attenuation")));
			P->SetStringField(TEXT("label"), ElemName);
			SubResult = HandleCreateAmbientSound(P);
		}
		else if (ElemType == TEXT("SOUND_CUE"))
		{
			TSharedPtr<FJsonObject> P = MakeShareable(new FJsonObject());
			P->SetStringField(TEXT("name"), ElemName);
			if (Elem->HasField(TEXT("sounds"))) P->SetArrayField(TEXT("sounds"), Elem->GetArrayField(TEXT("sounds")));
			if (Elem->HasField(TEXT("randomize"))) P->SetBoolField(TEXT("randomize"), Elem->GetBoolField(TEXT("randomize")));
			SubResult = HandleCreateSoundCue(P);
		}
		else if (ElemType == TEXT("SOUND_MIX"))
		{
			TSharedPtr<FJsonObject> P = MakeShareable(new FJsonObject());
			P->SetStringField(TEXT("name"), ElemName);
			if (Elem->HasField(TEXT("modifiers"))) P->SetArrayField(TEXT("modifiers"), Elem->GetArrayField(TEXT("modifiers")));
			SubResult = HandleCreateSoundMix(P);
		}
		else if (ElemType == TEXT("CONCURRENCY"))
		{
			TSharedPtr<FJsonObject> P = MakeShareable(new FJsonObject());
			P->SetStringField(TEXT("name"), ElemName);
			if (Elem->HasField(TEXT("max_count"))) P->SetNumberField(TEXT("max_count"), Elem->GetNumberField(TEXT("max_count")));
			if (Elem->HasField(TEXT("resolution_rule"))) P->SetStringField(TEXT("resolution_rule"), Elem->GetStringField(TEXT("resolution_rule")));
			SubResult = HandleSetSoundConcurrency(P);
		}
		else
		{
			SubResult = FCommandResult::Error(FString::Printf(TEXT("Unknown sound element type: %s"), *ElemType));
		}

		TSharedPtr<FJsonObject> ItemResult = MakeShareable(new FJsonObject());
		ItemResult->SetStringField(TEXT("type"), ElemType);
		ItemResult->SetStringField(TEXT("name"), ElemName);
		ItemResult->SetBoolField(TEXT("success"), SubResult.bSuccess);
		if (!SubResult.bSuccess) ItemResult->SetStringField(TEXT("error"), SubResult.ErrorMessage);
		Results.Add(MakeShareable(new FJsonValueObject(ItemResult)));

		if (SubResult.bSuccess) Succeeded++; else Failed++;
	}

	TSharedPtr<FJsonObject> Data = MakeShareable(new FJsonObject());
	Data->SetStringField(TEXT("design_name"), DesignName);
	Data->SetNumberField(TEXT("total"), Elements->Num());
	Data->SetNumberField(TEXT("succeeded"), Succeeded);
	Data->SetNumberField(TEXT("failed"), Failed);
	Data->SetArrayField(TEXT("results"), Results);
	return (Failed == 0) ? FCommandResult::Ok(Data) : FCommandResult::Ok(Data);
}
...
FCommandResult FCommandServer::HandleTestCreateNiagaraSystem(const TSharedPtr<FJsonObject>& Params)
{
	...
	auto Log = [&Steps](const FString& M) { Steps.Add(M); UE_LOG(LogArcwright, Log, TEXT("Niagara: %s"), *M); };
...
FCommandResult FCommandServer::HandleTestCreateMaterialGraph(const TSharedPtr<FJsonObject>& Params)
{
	...
	TArray<FString> Steps;
	auto Log = [&Steps](const FString& M) { Steps.Add(M); UE_LOG(LogArcwright, Log, TEXT("MatGraph: %s"), *M); };
...
FCommandResult FCommandServer::HandleTestCreateAnimBlueprint(const TSharedPtr<FJsonObject>& Params)
{
	...
	auto Log = [&Steps](const FString& Msg) { Steps.Add(Msg); UE_LOG(LogArcwright, Log, TEXT("AnimBP Test: %s"), *Msg); };
