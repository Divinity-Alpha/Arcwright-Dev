	...
	const TSharedPtr<FJsonObject>& IRJson, const FString& PackagePath)
{
	...
	UE_LOG(LogBlueprintLLM, Log, TEXT("BehaviorTree created: %s (%d composites, %d tasks, %d decorators, %d services)"),
	...
	const FString& PackagePath)
{
	...
	for (const TSharedPtr<FJsonValue>& KeyVal : Keys)
	{
		...
		{
			UE_LOG(LogBlueprintLLM, Warning, TEXT("Unknown blackboard key type: %s for key %s, defaulting to Object"), *KeyType, *KeyName);
			KeyTypeClass = UBlackboardKeyType_Object::StaticClass();
		}
		...
		UE_LOG(LogBlueprintLLM, Log, TEXT("  Blackboard key: %s (%s)"), *KeyName, *KeyType);
	}
	...
	UE_LOG(LogBlueprintLLM, Log, TEXT("Blackboard created: %s with %d keys"), *Name, BBAsset->Keys.Num());
	...
	FBTBuildResult& OutResult)
{
	...
	if (Node)
	{
		...
		UE_LOG(LogBlueprintLLM, Log, TEXT("  Composite: %s (%s)"), *NodeName, *DslType);
	}
	...
	FBTBuildResult& OutResult)
{
	...
	else
	{
		...
		UE_LOG(LogBlueprintLLM, Warning, TEXT("  Task '%s' mapped to Wait (custom tasks need Blueprint implementation)"), *DslType);
	}
	...
	if (Node)
	{
		...
		OutResult.TaskCount++;
		UE_LOG(LogBlueprintLLM, Log, TEXT("  Task: %s"), *DslType);
	}
	...
	FBTBuildResult& OutResult)
{
	...
	else
	{
		...
		Node = NewObject<UBTDecorator_ForceSuccess>(TreeAsset);
		UE_LOG(LogBlueprintLLM, Warning, TEXT("  Decorator '%s' not mapped — using ForceSuccess placeholder"), *DslType);
	}
	...
	if (Node)
	{
		...
		UE_LOG(LogBlueprintLLM, Log, TEXT("  Decorator: %s"), *DslType);
	}
	...
	FBTBuildResult& OutResult)
{
	...
	else
	{
		...
		UE_LOG(LogBlueprintLLM, Warning, TEXT("  Service '%s' not mapped — using DefaultFocus placeholder"), *DslType);
	}
	...
	if (Node)
	{
		...
		UE_LOG(LogBlueprintLLM, Log, TEXT("  Service: %s"), *DslType);
	}
...
UBehaviorTree* FBehaviorTreeBuilder::FindBehaviorTreeByName(const FString& Name)
{
	...
	TArray<FString> SearchPaths = {
		FString::Printf(TEXT("/Game/Arcwright/BehaviorTrees/%s.%s"), *Name, *Name),
