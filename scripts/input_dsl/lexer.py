...
def tokenize(dsl: str) -> List[Token]:
    ...
    for i, raw in enumerate(dsl.split("\n")):
        ...
        if m: tokens.append(Token("ACTION", name=m.group(1), line_num=ln)); continue
        m = re.match(r'^MAPPING:\\s*(\\S+)', stripped)
        if m: tokens.append(Token("MAPPING", name=m.group(1), line_num=ln)); continue
        m = re.match(r'^MAP:\\s*(\\S+)', stripped)
        if m: tokens.append(Token("MAP", name=m.group(1), line_num=ln)); continue
