...
def tokenize(dsl: str) -> List[Token]:
    ...
    for i, raw in enumerate(dsl.split(chr(10))):
        ...
        m = re.match(r"^PROC_MESH:\s*(\S+)", stripped)
        if m: tokens.append(Token("HEADER", name=m.group(1), line_num=ln)); continue
        m = re.match(r"^SHAPE:\s*(\S+)", stripped)
        if m: tokens.append(Token("SHAPE", name=m.group(1), line_num=ln)); continue
        m = re.match(r"^DEFORM:\s*(\S+)", stripped)
        if m: tokens.append(Token("DEFORM", name=m.group(1), line_num=ln)); continue
