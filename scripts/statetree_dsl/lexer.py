...
def tokenize(dsl: str) -> List[Token]:
    ...
    for i, raw in enumerate(dsl.split("\n")):
        ...
        m = re.match(r'^STATE_TREE:\\s*(\\S+)', stripped)
        if m: tokens.append(Token("HEADER", name=m.group(1), line_num=ln)); continue
        m = re.match(r'^STATE:\\s*(\\S+)', stripped)
        if m: tokens.append(Token("STATE", name=m.group(1), line_num=ln)); continue
        m = re.match(r'^TASK:\\s*(\\S+)', stripped)
        if m: tokens.append(Token("TASK", name=m.group(1), line_num=ln)); continue
        m = re.match(r'^CONDITION:\\s*(\\S+)', stripped)
        if m: tokens.append(Token("CONDITION", name=m.group(1), line_num=ln)); continue
        m = re.match(r'^CONSIDERATION:\\s*(\\S+)', stripped)
        if m: tokens.append(Token("CONSIDERATION", name=m.group(1), line_num=ln)); continue
