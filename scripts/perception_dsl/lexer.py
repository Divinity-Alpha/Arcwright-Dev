...
def tokenize(dsl: str) -> List[Token]:
    ...
    for i, raw in enumerate(dsl.split("\n")):
        ...
        m = re.match(r'^PERCEPTION:\s*(\S+)', stripped)
        if m: tokens.append(Token("HEADER", name=m.group(1), line_num=ln)); continue
        m = re.match(r'^SENSE:\s*(\S+)', stripped)
        if m: tokens.append(Token("SENSE", name=m.group(1), line_num=ln)); continue
        m = re.match(r'^TEAM:', stripped)
        if m: tokens.append(Token("TEAM", line_num=ln)); continue
