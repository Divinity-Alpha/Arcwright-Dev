...
def tokenize(dsl: str) -> List[Token]:
    ...
    for i, raw in enumerate(dsl.split("\n")):
        ...
        m = re.match(r'^SMART_OBJECT:\\s*(\\S+)', stripped)
        if m: tokens.append(Token("HEADER", name=m.group(1), line_num=ln)); continue
        m = re.match(r'^SLOT:\\s*(\\S+)', stripped)
        if m: tokens.append(Token("SLOT", name=m.group(1), line_num=ln)); continue
