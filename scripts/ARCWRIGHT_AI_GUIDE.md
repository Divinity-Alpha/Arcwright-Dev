...
widgets created outside Arcwright (by artists, designers, or the C++ constructor
system), it **DESTROYS** the internal structure — event graphs, bindings, custom
logic, and hierarchy relationships.

A 170KB widget can shrink to 45KB — a **74% content loss** that breaks everything.

**SAFE:**
- `set_widget_property` on widgets created by Arcwright's `create_widget_blueprint` ✓
- Reading widget data via `get_widget_tree` on any widget ✓

**DANGEROUS:**
- `set_widget_property` on `WBP_MainHUD` (created by C++ system) ✗
- `set_widget_property` on any widget not in `/Game/UI/` ✗

**RULE: If you didn't create it, don't modify it.** Read it, inspect it, but
don't write to it.

If you need to modify an external widget at runtime, use C++ code:
```cpp
// Safe: find a child widget and set its text/visibility at runtime
UTextBlock* Prompt = Cast<UTextBlock>(Widget->WidgetTree->FindWidget(FName("txt_InteractionPrompt")));
if (Prompt) {
    Prompt->SetText(FText::FromString("Press E — Station Name"));
    Prompt->SetVisibility(ESlateVisibility::HitTestInvisible);
}
```

The `set_widget_property` command now blocks modifications to widgets outside `/Game/UI/`
and requires `force: true` to override. This prevents accidental destruction.
