"""Build styled station widgets in BoreAndStroke — v2 with correct Arcwright property names."""
import socket
import json
import time
import sys
...
def build_station(wb, title, desc):
    """Delete and rebuild one station widget with correct properties."""
    ...
    # ── Border_Panel (root child — fills entire screen) ──
    add(wb, "Border", "Border_Panel")
    prop(wb, "Border_Panel", "BrushColor", C_PANEL_BG)
    prop(wb, "Border_Panel", "Brush.DrawType", "Box")
    # Canvas slot: anchor 0,0 → 1,1 so it fills any resolution
    prop(wb, "Border_Panel", "Slot.Anchors.Min.X", "0.0")
    prop(wb, "Border_Panel", "Slot.Anchors.Min.Y", "0.0")
    prop(wb, "Border_Panel", "Slot.Anchors.Max.X", "1.0")
    prop(wb, "Border_Panel", "Slot.Anchors.Max.Y", "1.0")
    prop(wb, "Border_Panel", "Slot.AutoSize", "true")
    ...
    # Don't fill vertical space — use auto height
    ...
    prop(wb, "txt_Title", "Font.Size", "28")
    ...
    prop(wb, "txt_ItemInfo", "Font.Size", "20")
    ...
    prop(wb, "txt_ActionsHeader", "Font.Size", "16")
    ...
    prop(wb, "txt_Actions", "Font.Size", "20")
    ...
    prop(wb, "txt_Equipment", "Font.Size", "18")
    ...
    prop(wb, "txt_ExitHint", "Font.Size", "16")
...
if __name__ == "__main__":
    print(f"=== Building {len(STATIONS)} station widgets (v2) ===\n")
