...
def execute(commands, host="localhost", port=13377, timeout=60.0, dry_run=False):
    ...
    if dry_run:
        for cmd in commands:
            results.append({"command": cmd["command"], "status": "dry_run"}); ok += 1
    ...
    try:
        for cmd in commands:
            try:
                ...
                s = resp.get("status", "?"); results.append({"command": cmd["command"], "status": s})
                if s == "ok": ok += 1
                else: fail += 1
            except Exception as e:
                results.append({"command": cmd["command"], "status": "error", "message": str(e)}); fail += 1
