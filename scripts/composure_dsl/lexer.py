...
def tokenize(dsl: str) -> List[Token]:
    ...
    for i, raw in enumerate(dsl.split(chr(10))):
        ...
        m = re.match(r"^COMPOSURE:\s*(\S+)", stripped)
        if m: tokens.append(Token("HEADER", name=m.group(1), line_num=ln)); continue
        m = re.match(r"^ELEMENT:\s*(\S+)", stripped)
        if m: tokens.append(Token("ELEMENT", name=m.group(1), line_num=ln)); continue
        m = re.match(r"^PASS:\s*(\S+)", stripped)
        if m: tokens.append(Token("PASS", name=m.group(1), line_num=ln)); continue
        m = re.match(r"^COMPOSITE:\s*(\S+)", stripped)
        if m: tokens.append(Token("COMPOSITE", name=m.group(1), line_num=ln)); continue
