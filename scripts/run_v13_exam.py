"""
Run full v13 exam suite across ALL 21 lessons sequentially, reusing model between lessons.
Saves individual lesson results + combined summary.
v13 = full consolidation training (21 lessons, 389 prompts, golden config).
"""
...
def main():
    ...
    parser.add_argument("--model", default="models/blueprint-lora-v13/final", help="Model path")
    ...
    # Find all lesson files L01-L21
    lesson_files = sorted(glob.glob(f"{lessons_dir}/lesson_*.json"))
    lesson_files = [f for f in lesson_files if any(f"lesson_{i:02d}.json" in f for i in range(args.start_lesson, 22))]

    print(f"{'='*60}")
    print(f"  FULL EXAM SUITE — v13 (full consolidation, 21 lessons)")
    ...
    for i, lesson_path in enumerate(lesson_files):
        ...
        results_file = Path(output_dir) / f"exam_{lesson_id}_v13_{ts}.jsonl"
        summary_file = Path(output_dir) / f"exam_{lesson_id}_v13_{ts}_summary.json"
    ...
    combined_file = Path(output_dir) / f"exam_all_summaries_v13_{combined['timestamp']}.json"
    with open(combined_file, "w", encoding="utf-8") as f:
        json.dump(combined, f, indent=2, ensure_ascii=False)

    print(f"\n{'='*60}")
    print(f"  FULL SUITE RESULTS — v13 (full consolidation, 21 lessons)")
    print(f"{'='*60}")
    for s in all_summaries:
        syn_flag = " *NEW*" if s['lesson_id'] in ('lesson_19', 'lesson_20', 'lesson_21') else ""
        print(f"  {s['lesson_id']:>12}: syntax={s['valid_syntax_pct']:5.1f}% ({s['valid_syntax']:2}/{s['total_prompts']:2})  sim={s['avg_similarity_score']:5.1f}%{syn_flag}")
    print(f"  {'':>12}  {'─'*40}")
