...
def _create_single_asset(template: dict, customs: dict, client, spawn_loc):
    ...
    if mat:
        ...
        if mesh_comps:
            try:
                client.apply_material(bp_name, mesh_comps[0]["name"], f"/Game/BlueprintLLM/Materials/{mat_name}")
...
def _create_multi_asset(template: dict, customs: dict, client, spawn_loc):
        ...
        if "blueprint_dsl" in asset_def:
            ...
            if mat:
                try:
                    ...
                    if mesh_comps:
                        client.apply_material(bp_name, mesh_comps[0]["name"], f"/Game/BlueprintLLM/Materials/{mat['name']}")
    ...
    if spawn_loc:
        ...
        if pawn_key and "blueprint_dsl" in assets[pawn_key]:
            ...
            label = customs.get("label", f"{bp_name}_1")
            try:
                client.spawn_actor_at(f"/Game/BlueprintLLM/Generated/{bp_name}",
